...
Since integers are 16 bits on the Arduino, the possibility of overflow exists.
It is pretty easy to end up with a floating point number, though, so this can be
avoided easily at the cost of performance.
...
concatenate:with: | Join ()()   | y                     | Concatenation is currently very inefficient
...
putPenUp        | Pen Up        | n                     |
randomFrom:to:  | Pick Random () to () | y              | Random number generator isn't seeded, so these aren't very good random numbers
